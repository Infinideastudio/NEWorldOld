...
pub struct Frustum<S> {
    ...
    pub planes: [Vector4<S>; 6],
}

impl<S: BaseFloat> Frustum<S> {
    /// Extract the six frustum planes from a model-view-projection matrix.
    /// Same row-combination form as the C++ build; assumes column-vector
    /// convention (`clip = mvp * point`), which cgmath matches.
    #[must_use]
    pub fn from_mvp(mvp: &Matrix4<S>) -> Self {
        let r0 = mvp.row(0);
        let r1 = mvp.row(1);
        let r2 = mvp.row(2);
        let r3 = mvp.row(3);
    ...
    /// lies entirely outside one of the planes. Mirrors the C++ predicate.
    #[must_use]
    pub fn test(&self, box_: &Aabb3<S>) -> bool {
        let bmin = box_.min;
        let bmax = box_.max;
        let one = S::one();
        let zero = S::zero();
...
mod tests {
    ...
    fn frustum_culls_box_behind_camera() {
        ...
        let behind = Aabb3::<f32>::new(Vector3::new(-1.0, -1.0, 5.0), Vector3::new(1.0, 1.0, 10.0));
        assert!(!f.test(&behind));

        // Off to the side, far past the X frustum bounds at this depth.
        let way_left = Aabb3::<f32>::new(
    ...
    fn frustum_passes_box_around_origin() {
        ...
        let around = Aabb3::<f32>::new(Vector3::new(-2.0, -2.0, -2.0), Vector3::new(2.0, 2.0, 2.0));
