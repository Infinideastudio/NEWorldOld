...
struct fstring { // NOLINT
    ...
    template <size_t M>
    consteval auto operator==(fstring<M> const& r) const -> bool {
        if constexpr (N != M) {
            return false;
        }
    ...
    requires (I + L <= N)
    consteval auto substr() const -> fstring<L> {
        auto res = fstring<L>();
    ...
    */
    ...
    template <size_t I>
    requires (I <= N)
    consteval auto suffix() const -> fstring<N - I> {
        auto res = fstring<N - I>();
