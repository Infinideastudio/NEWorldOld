...
// The dimension is `depth * height * width`.
...
public:
    ...
    void fill(size_t layer, size_t row, size_t column, Image const& src, size_t src_layer = 0) {
        assert(row + src.height() <= height() && column + src.width() <= width());
    ...
    // Compatibility method: flips every layer vertically.
    void flip() {
        for (auto i = 0uz; i < depth(); i++) {
            for (auto j = 0uz; j < height() / 2; j++) {
                for (auto k = 0uz; k < width(); k++) {
                    std::swap(_dims[i, j, k], _dims[i, height() - 1 - j, k]);
                }
            }
        }
    }
...
export auto save_png_image(std::filesystem::path const& filename, ImageRGBA const& image)
    ...
    if (setjmp(png_jmpbuf(writer.get())) == 0) {
        ...
        // Write image data.
        for (auto i = 0uz; i < image.height(); i++) {
