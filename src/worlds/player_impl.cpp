...
void Player::updatePosition(worlds::World& world, double timeDelta) {
    ...
    if (!crossWall) {
        hitboxes.clear();
        hitboxes = world.hitboxes(Hitbox::Expand(playerbox, velocity.x, velocity.y, velocity.z));
    ...
    if (onGround) {
        if (jump < -0.005) {
            ...
            if (heightExt <= -0.005) {
                heightExt *= std::pow(0.8f, timeDelta * 30.0f);
            }
        }
    }
    ...
    velocity = (velocity * 100000).round() / 100000.0;
...
bool Player::putBlock(worlds::World& world, Vec3i coord, blocks::Id blockname) {
    Hitbox::AABB blockbox{coord.x - 0.5, coord.y - 0.5, coord.z - 0.5, coord.x + 0.5, coord.y + 0.5, coord.z + 0.5};
