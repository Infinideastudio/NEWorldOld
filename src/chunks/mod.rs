...
impl Chunk {
    ...
    /// Total per-cell data size in bytes.
    pub const SIZE_DATA: usize = Self::SIZE_CUBED * core::mem::size_of::<BlockData>();
    ...
    fn cell_mut(&mut self, idx: usize) -> &mut BlockData {
        self.data
            .as_mut()
    ...
    /// followed by `SIZE_DATA` bytes of `BlockData`. Returns an empty `Vec`
    /// for empty chunks (callers should not save empty chunks).
    #[must_use]
    pub fn package_to(&self) -> Vec<u8> {
        ...
        out.extend_from_slice(bytemuck::cast_slice::<BlockData, u8>(data.as_ref()));
        out
    }
    ...
    pub fn unpackage_from(&mut self, bytes: &[u8]) -> Result<(), ChunkError> {
        ...
        let header: Header = *bytemuck::from_bytes::<Header>(head);
...
mod tests {
    ...
    fn sizes_match_constants() {
        ...
        assert_eq!(Chunk::SIZE_DATA, 4096 * core::mem::size_of::<BlockData>());
    }
