...
private:
    ...
    GUI::Trackbar shadowresbar =
        GUI::Trackbar("", 120, (int) (log2(Renderer::ShadowRes) - 10) * 40 - 1, 10, 250, 60, 84, 0.5, 0.5, 0.0, 0.0);
    GUI::Trackbar shadowdistbar =
        GUI::Trackbar("", 120, (Renderer::MaxShadowDistance - 4) * 6 - 1, -250, -10, 96, 120, 0.5, 0.5, 0.0, 0.0);
    ...
    void onUpdate() {
        ...
        if (softshadowbtn.clicked)
            Renderer::SoftShadow = !Renderer::SoftShadow;
        if (cloudsbtn.clicked)
            Renderer::VolumetricClouds = !Renderer::VolumetricClouds;
        if (ssaobtn.clicked)
            Renderer::AmbientOcclusion = !Renderer::AmbientOcclusion;
        if (backbtn.clicked)
            exit = true;
        shadowresbar.enabled = shadowdistbar.enabled = softshadowbtn.enabled = cloudsbtn.enabled = ssaobtn.enabled =
            Renderer::AdvancedRender;
    }
};
