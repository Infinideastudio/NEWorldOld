...
public:
    struct Args {
        ...
        std::shared_ptr<render::Texture const> texture;
    };

    explicit ImageBox(Args&& args): // NOLINT
        _alignment(alignment_to_fractions(args.alignment)),
        _fit(args.fit),
        _texture(std::move(args.texture)) {}
...
private:
    ...
    std::shared_ptr<render::Texture const> _texture;
