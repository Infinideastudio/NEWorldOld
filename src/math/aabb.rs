...
impl<T: BaseFloat> Aabb<T> {
    ...
    pub fn intersects_on_axis(&self, i: usize, other: &Self, eps: T) -> bool {
        let a = self.min[i];
        let b = self.max[i];
    ...
    pub fn intersects(&self, other: &Self, eps: T) -> bool {
        (0..3).all(|i| self.intersects_on_axis(i, other, eps))
    }

    /// Single-axis displacement clip against one box. See `clip_displacement`.
    pub fn clip_displacement_on_axis(&self, i: usize, other: &Self, disp: T, eps: T) -> T {
        // TODO (need fix): floating point operations are not associative
        let zero = T::zero();
        for j in 0..3 {
    ...
    pub fn clip_displacement(&self, others: &[Self], mut disp: Vector3<T>, eps: T) -> Vector3<T> {
        // TODO (need fix): floating point operations are not associative
        let mut t = *self;
        for i in 0..3 {
            let mut d = disp[i];
    ...
    pub fn extend(&self, delta: Vector3<T>) -> Self {
        let zero = T::zero();
...
impl<T: BaseFloat> Default for Aabb<T> {
    fn default() -> Self {
        Self {
            min: Vector3::zero(),
            max: Vector3::zero(),
...
impl<T: BaseFloat> Add<Vector3<T>> for Aabb<T> {
    type Output = Self;
...
impl<T: BaseFloat> Sub<Vector3<T>> for Aabb<T> {
    type Output = Self;
...
impl<T: BaseFloat> AddAssign<Vector3<T>> for Aabb<T> {
    fn add_assign(&mut self, rhs: Vector3<T>) {
        *self = *self + rhs;
    }
}

impl<T: BaseFloat> SubAssign<Vector3<T>> for Aabb<T> {
...
mod tests {
    ...
    #[test]
    fn intersects_basic() {
        let a = Aabb::<f64>::new(Vector3::new(0.0, 0.0, 0.0), Vector3::new(1.0, 1.0, 1.0));
        let b = Aabb::<f64>::new(Vector3::new(0.5, 0.5, 0.5), Vector3::new(1.5, 1.5, 1.5));
        let c = Aabb::<f64>::new(Vector3::new(2.0, 2.0, 2.0), Vector3::new(3.0, 3.0, 3.0));
        assert!(a.intersects(&b, 0.0));
        assert!(!a.intersects(&c, 0.0));
        // Touching at a face is not an intersection (open-interval semantics).
        let d = Aabb::<f64>::new(Vector3::new(1.0, 0.0, 0.0), Vector3::new(2.0, 1.0, 1.0));
    ...
    fn clip_displacement_2d_equivalent() {
        ...
        let mover = Aabb::<f64>::new(
            Vector3::new(0.0, 0.0, -100.0),
            Vector3::new(1.0, 1.0, 100.0),
        );
        let wall = Aabb::<f64>::new(Vector3::new(2.0, 0.1, -50.0), Vector3::new(3.0, 0.9, 50.0));
    ...
    fn clip_displacement_3d_corner_collision() {
        ...
        let player = Aabb::<f64>::new(
            Vector3::new(0.05, 0.05, 0.05),
            Vector3::new(0.55, 0.85, 0.55),
        );
        let block_x = Aabb::<f64>::new(Vector3::new(1.0, 0.0, 0.0), Vector3::new(2.0, 1.0, 1.0));
        let block_z = Aabb::<f64>::new(Vector3::new(0.0, 0.0, 1.0), Vector3::new(1.0, 1.0, 2.0));
        ...
        let floor = Aabb::<f64>::new(
            Vector3::new(-10.0, -1.0, -10.0),
            Vector3::new(10.0, 0.0, 10.0),
        );
        let player_above =
            Aabb::<f64>::new(Vector3::new(0.05, 0.5, 0.05), Vector3::new(0.55, 2.3, 0.55));
    ...
    fn extend_grows_box_in_motion_direction() {
        let b = Aabb::<f64>::new(Vector3::new(0.0, 0.0, 0.0), Vector3::new(1.0, 1.0, 1.0));
