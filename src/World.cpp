    ...
    if (cptr != nullptr) {
        ...
        if (oldblock == Blocks::GLOWSTONE || oldblock == Blocks::LAVA) {
            ...
            if (blks[maxbrightness] == Blocks::WATER) {
                if (br - 2 < MinBrightness)
                    br = MinBrightness;
                else
                    br -= 2;
            } else {
                if (br - 1 < MinBrightness)
                    br = MinBrightness;
                else
                    br--;
            }
        ...
        if (oldblock != cptr->getBlock(bx, by, bz))
            updated = true;
        ...
        if (updated) {
            ...
            if (bx == 15 && cx < WorldSize - 1)
                markChunkNeighborUpdated(cx + 1, cy, cz);
            if (bx == 0 && cx > -WorldSize)
                markChunkNeighborUpdated(cx - 1, cy, cz);
            if (by == 15 && cy < WorldHeight - 1)
                markChunkNeighborUpdated(cx, cy + 1, cz);
            if (by == 0 && cy > -WorldHeight)
                markChunkNeighborUpdated(cx, cy - 1, cz);
            if (bz == 15 && cz < WorldSize - 1)
                markChunkNeighborUpdated(cx, cy, cz + 1);
...
Brightness getBrightness(int x, int y, int z) {
    ...
    auto cptr = getChunkPtr(cx, cy, cz);
    if (cptr == EmptyChunkPtr)
        return cy < 0 ? MinBrightness : SkyBrightness;
    if (cptr != nullptr)
    ...
    for (int radius = int(loadedCore.radius) + 1; radius <= RenderDistance + 1; radius++) {
        ...
        for (int cx = cxp - radius; cx < cxp + radius; cx++) {
            for (int cy = cyp - radius; cy < cyp + radius; cy++) {
                ...
                if (cx == cxp - radius || cx == cxp + radius - 1)
                    stride = 1;
                if (cy == cyp - radius || cy == cyp + radius - 1)
                    stride = 1;
    ...
    for (int xt = 0; xt != 5; xt++) {
        for (int zt = 0; zt != 5; zt++) {
            for (int yt = 0; yt != 2; yt++) {
                if (getBlock(x + xt - 2, y + th - 3 + yt, z + zt - 2) == Blocks::AIR)
                    setBlock(x + xt - 2, y + th - 3 + yt, z + zt - 2, Blocks::LEAF);
            }
        }
    }
