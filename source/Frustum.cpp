...
namespace Frustum
{
    ...
    bool AABBInFrustum(const Hitbox::AABB &aabb)
    {
        ...
        {
            if (frus[i + 0] * aabb.min.x + frus[i + 1] * aabb.min.y + frus[i + 2] * aabb.min.z + frus[i + 3] <= 0.0 &&
                frus[i + 0] * aabb.max.x + frus[i + 1] * aabb.min.y + frus[i + 2] * aabb.min.z + frus[i + 3] <= 0.0 &&
                frus[i + 0] * aabb.min.x + frus[i + 1] * aabb.max.y + frus[i + 2] * aabb.min.z + frus[i + 3] <= 0.0 &&
                frus[i + 0] * aabb.max.x + frus[i + 1] * aabb.max.y + frus[i + 2] * aabb.min.z + frus[i + 3] <= 0.0 &&
                frus[i + 0] * aabb.min.x + frus[i + 1] * aabb.min.y + frus[i + 2] * aabb.max.z + frus[i + 3] <= 0.0 &&
                frus[i + 0] * aabb.max.x + frus[i + 1] * aabb.min.y + frus[i + 2] * aabb.max.z + frus[i + 3] <= 0.0 &&
                frus[i + 0] * aabb.min.x + frus[i + 1] * aabb.max.y + frus[i + 2] * aabb.max.z + frus[i + 3] <= 0.0 &&
                frus[i + 0] * aabb.max.x + frus[i + 1] * aabb.max.y + frus[i + 2] * aabb.max.z + frus[i + 3] <= 0.0)
            {
                return false;
            }
        }
