...
namespace Menus {
    ...
    private:
        ...
        GUI::Trackbar msaabar = GUI::Trackbar("", 120, Multisample == 0 ? 0 : (int)(log2(Multisample) - 1) * 40 - 1, 10, 250, 96, 120, 0.5, 0.5, 0.0, 0.0);
        ...
        void onLoad() {
            title.centered = true;
            registerControls({ &title, &smoothlightingbtn, &fancygrassbtn, &mergefacebtn, &msaabar, &vsyncbtn, &shaderbtn, &backbtn });
        }
        ...
        void onUpdate() {
            ...
            msaabar.text = GetStrbyKey("NEWorld.render.multisample") + (Multisample != 0 ? Var2Str(Multisample) + "x" : BoolEnabled(false));
            ...
            if (vsyncbtn.clicked) VerticalSync = !VerticalSync;
