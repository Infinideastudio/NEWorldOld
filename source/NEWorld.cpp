				...
				if (mb == 1 || glfwGetKey(MainWindow, GLFW_KEY_ENTER) == GLFW_PRESS) {
					...
					if (selx != oldselx || sely != oldsely || selz != oldselz) { seldes = 0.0; }
						...
						if (Player::gamemode==Player::Creative)
						{
							...
							seldes += BlockInfo(selb).getHardness()*0.2;
						}
					BlockClick = true;
		...
		if (!Player::inWater) {
			if (!Player::Flying && !Player::CrossWall) {
				...
				if (Player::OnGround) {
					...
					/*if (Player::t==0)
					{
						Player::t = GetTickCount64();
					}
					else
					{
							Player::jump -= 0.0098* double(GetTickCount64() - Player::t) / 30.0;//速度/UPS
							if (Player::jump < -53.6 / 30.0)Player::jump = -53.6 / 30.0;//在高空, 人体下落的终端速度极限为53.6米每秒
							Player::ya = Player::jump + 0.5 * 0.6 / 900.0;
					}*/
					//好吧我认输，第二次还是不行
				}
