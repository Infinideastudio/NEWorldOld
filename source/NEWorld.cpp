	...
	if (!bagOpened)
	{
		...
		for (int i = 0; i < selectPrecision*selectDistance; i++)
		{
			...
			if (BlockInfo (world::getblock (RoundInt (lx), RoundInt (ly), RoundInt (lz))).isSolid ())
			{
				...
				if (((mb == 2 && mbp == false) || isPressed (GLFW_KEY_TAB)))
				{ //鼠标右键
...
void drawBag ()
{
	...
	unsigned short itemHovering = player::inventory [si] [sj];
	if (itemHovering != 0 && sf == 1)
	{
		glColor4f (1.0, 1.0, 0.0, 1.0);
		if (isBlock (itemHovering))
		{
		...
		else
		{
			//TODO:输出物品的名称而不是ID
			std::stringstream ss;
			ss << itemsinfo [itemHovering - 30000].id;
			TextRenderer::renderString ((int) mx, (int) my - 16, ss.str ());
		}
	}
	mousebl = mouseb;
}
