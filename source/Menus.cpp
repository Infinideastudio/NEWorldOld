...
private:
	...
	void onLoad() {
		...
		mergefacebtn = gui::button("合并面渲染：" + boolstr(MergeFace), -250, -10, 96, 120, 0.5, 0.5, 0.0, 0.0);
	...
	void onUpdate() {
		...
		mergefacebtn.text = "合并面渲染：" + boolstr(MergeFace);
	}
};
...
private:
	gui::label title;
	gui::button fontbtn, backbtn;
	void onLoad() {
		title = gui::label("===============< 图形界面选项 >==============", -225, 225, 20, 36, 0.5, 0.5, 0.0, 0.0);
		fontbtn = gui::button("全部使用Unicode字体：" + boolstr(TextRenderer::useUnicodeASCIIFont), -250, -10, 60, 84, 0.5, 0.5, 0.0, 0.0);
		backbtn = gui::button("<< 返回选项菜单", -250, 250, -44, -20, 0.5, 0.5, 1.0, 1.0);
		registerControls(3, &title, &fontbtn, &backbtn);
	}
	void onUpdate() {
