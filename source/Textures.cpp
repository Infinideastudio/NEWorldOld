...
namespace Textures
{
    ...
    void LoadRGBAImage(TEXTURE_RGBA &tex,const string& Filename,const string& MkFilename)
    {
        ...
        if (!noMaskFile)
        {
            maskfile.open(MkFilename, std::ios::binary | std::ios::in);    //����λͼ�ļ��������ƣ�
        }
    ...
    TextureID LoadFontTexture(int id)
    {
        ...
        std::ifstream bmpfile(s.str(), std::ios::binary); //λͼ�ļ��������ƣ�

        if (!bmpfile.is_open())
        {
            DebugWarning("Cannot load bitmap " + s.str());
            return 0;
        }
