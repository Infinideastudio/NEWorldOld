...
inline void FrustumTest::MultMatrix(float * a, float * b) {
	float sum[16]; MultMatrixTo(sum, a, b);
	memcpy(a, sum, sizeof(sum));
}

void FrustumTest::SetPerspective(float FOV, float aspect, float Znear, float Zfar) {
...
void FrustumTest::SetOrtho(float left, float right, float top, float bottom, float Znear, float Zfar) {
	proj[0] = 2 / (right - left);
	proj[5] = 2 / (bottom - top);
