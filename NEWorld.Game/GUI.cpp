...
namespace GUI {
    ...
    void button::update() {
        ...
        clicked = ((parent->mb == 0 && parent->mbl == 1 && mouseon) || (parent->enterpl && !parent->enterp)) && focused;
        //clicked = lp&&!pressed
    ...
    void trackbar::update() {
        ...
        if (parent->mx >= xmin && parent->mx <= xmax && parent->my >= ymin && parent->my <= ymax && parent->mb == 1)
            parent->focusid = id;
        mouseon = parent->mx >= xmin + barpos && parent->mx <= xmin + barpos + barwidth && parent->my >= ymin &&
                  parent->my <= ymax;
    ...
    void vscroll::update() {
        ...
        //更新滚动条状态
        //鼠标悬停
        mouseon = (parent->my >= ymin + barpos + 20 && parent->my <= ymin + barpos + barheight + 20 &&
                   parent->mx >= xmin && parent->mx <= xmax);
        if (parent->mx >= xmin && parent->mx <= xmax && parent->my >= ymin && parent->my <= ymax) {
    ...
    void Form::update() {
        ...
        if (glfwGetKey(MainWindow, GLFW_KEY_TAB) == GLFW_PRESS) {                             //TAB键切换焦点
            if (glfwGetKey(MainWindow, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS ||
                glfwGetKey(MainWindow, GLFW_KEY_RIGHT_SHIFT) == GLFW_PRESS) {   //Shift+Tab
                updated = true;
                if (!tabp) focusid--;
    ...
    button::button(std::string t, int xi_r, int xa_r, int yi_r, int ya_r, double xi_b, double xa_b, double yi_b,
                   double ya_b)
            : button() {
        text = t;
        enabled = true;
    ...
    trackbar::trackbar(std::string t, int w, int s, int xi_r, int xa_r, int yi_r, int ya_r, double xi_b, double xa_b,
    ...
    textbox::textbox(std::string t, int xi_r, int xa_r, int yi_r, int ya_r, double xi_b, double xa_b, double yi_b,
                     double ya_b)
            : textbox() {
        text = t;
        enabled = true;
